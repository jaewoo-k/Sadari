...
public class Hst_InfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			// 파일이 실제로 저장 될 경로
			String savePath = root + "resources\\uploadHouseImg\\";
			...
			while(files.hasMoreElements()) {
				...
				hp.setShIpath("/resources/uploadHouseImg/");
				...
				if(i >= 0) {
					...
					if(name.equals("thumbnailImg")) {
						 hp.setShItype("T");
					 }else if(name.contains("shareImg")) {
						 hp.setShItype("S");
					 }else if(name.contains("amenImg")) {
						 hp.setShItype("A");
					 }else if(name.contains("etcImg")) {
						 hp.setShItype("E");
					 }else {
						 hp.setShItype("R");
					 }
